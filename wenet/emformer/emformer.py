...
class EmformerAttention(nn.Module):
    ...
    def _gen_attention_probs(
        ...
        if padding_mask is not None:
            ...
            B = torch.div(attention_weights.size(0), self.nhead, rounding_mode='floor')
...
class EmformerEncoderLayer(nn.Module):
    ...
    def _update_attn_cache(
        ...
        """
        new_memory = torch.cat([pre_memory, memory])
        new_key = torch.cat([left_context_key, next_key])
        new_val = torch.cat([left_context_val, next_val])
        new_memory = new_memory[new_memory.size(0) - self.memory_size :]
        new_key = new_key[new_key.size(0) - self.left_context_length :]
        new_val = new_val[new_val.size(0) - self.left_context_length :]
        return new_memory, new_key, new_val
    ...
    def _apply_attention_module_infer(
        ...
        new_memory, new_key, new_val = self._update_attn_cache(
            next_key, next_val, memory, pre_memory, left_context_key, left_context_val
        )
        return output_right_context_utterance, new_memory, new_key, new_val
    ...
    def infer(
        ...
        # emformer attention module
        src_att, new_memory, new_key, new_val = self._apply_attention_module_infer(
            src, R, prev_memory, left_context_key, left_context_val, padding_mask=padding_mask
        )
        src = src + self.dropout(src_att)
...
class EmformerEncoder(nn.Module):
    ...
    *Emformer: Efficient Memory Transformer Based Acoustic Model for Low Latency
    ...
    def infer(
        ...
        output = utterance
        out_memory_caches = torch.zeros((0, 0, 0), dtype=output.dtype, device=output.device)
        out_left_key_caches = torch.zeros((0, 0, 0, 0), dtype=output.dtype, device=output.device)
        out_left_val_caches = torch.zeros((0, 0, 0, 0), dtype=output.dtype, device=output.device)
        output_conv_caches = torch.zeros((0, 0, 0), dtype=output.dtype, device=output.device)
        ...
        for layer_idx, layer in enumerate(self.emformer_layers):
            (
                ...
                output,
                right_context,
                prev_memory=memory_caches[layer_idx],
                left_context_key = left_key_caches[layer_idx],
                left_context_val = left_val_caches[layer_idx],
                conv_cache=conv_caches[layer_idx],
                padding_mask=padding_mask,
            )
            if layer_idx == 0:
                out_memory_caches = new_memory.unsqueeze(0)
                out_left_key_caches = new_key.unsqueeze(0)
                out_left_val_caches = new_val.unsqueeze(0)
                output_conv_caches = output_conv_cache.unsqueeze(0)
            else:
                out_memory_caches = torch.cat((out_memory_caches, new_memory.unsqueeze(0)), dim=0)
                out_left_key_caches = torch.cat([out_left_key_caches, new_key.unsqueeze(0)], dim=0)
                out_left_val_caches = torch.cat([out_left_val_caches, new_val.unsqueeze(0)], dim=0)
                output_conv_caches = torch.cat([output_conv_caches, output_conv_cache.unsqueeze(0)], dim=0)

        return output, output_lengths, out_memory_caches, out_left_key_caches, out_left_val_caches, output_conv_caches

    @torch.jit.export
    def init_states(self, device: torch.device = torch.device("cpu")):
        """Create initial states."""
        memory_caches = torch.zeros(self.num_encoder_layers, self.memory_size, self.d_model, device=device)
        left_key_caches = torch.zeros(
                    self.num_encoder_layers, self.left_context_length, self.d_model, device=device
                ),
        left_val_caches = torch.zeros(
                    self.num_encoder_layers, self.left_context_length, self.d_model, device=device
                ),
        conv_caches = torch.zeros(self.num_encoder_layers, self.d_model, self.cnn_module_kernel - 1, device=device)

        return memory_caches, left_key_caches, left_val_caches, conv_caches
